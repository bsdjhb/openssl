...
# if defined (__FreeBSD__)
    ...
    if (EVP_CIPHER_mode(c) == EVP_CIPH_GCM_MODE) {
	 ...
	 case NID_aes_256_cbc_hmac_sha1:
	      tls_en.auth_algorithm = CRYPTO_SHA1_HMAC;
	      break;
	 case NID_aes_128_cbc_hmac_sha256:
	 case NID_aes_256_cbc_hmac_sha256:
	      tls_en.auth_algorithm = CRYPTO_SHA2_256_HMAC;
	      break;
	 default:
	      goto skip_ktls;
        }
        tls_en.auth_key = ms;
	tls_en.auth_key_len = *mac_secret_size;
    } else
        goto skip_ktls;
    tls_en.cipher_key = key;
    tls_en.cipher_key_len = EVP_CIPHER_key_length(c);
    tls_en.iv = iv;
